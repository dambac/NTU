...
class Analyzer:
    def __init__(self, results_dir):
        ...
        self.output_path = f"{output_dir}/sorted_summary.csv"

    def analyze(self):
        df_data = []

        comb_results: List[CombinationResults] = self.results.combination_results
        comb_results = self.get_combination_results_that_are_sorted(comb_results)
    ...
    @staticmethod
    def get_combination_results_that_are_sorted(combination_results: List[CombinationResults]):
        results_sorted = []

        for c_results in combination_results:
            iteration_results_sorted = sorted(c_results.iterations_results,
                                              key=lambda i_results: sum(i_results.fit_results.train_metrics.f1_scores),
                                              reverse=True)
            results_sorted.append(CombinationResults(combination=c_results.combination,
                                                     dataset_results=c_results.dataset_results,
                                                     iterations_results=iteration_results_sorted))
