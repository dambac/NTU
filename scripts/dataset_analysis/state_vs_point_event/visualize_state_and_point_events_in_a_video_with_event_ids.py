...
def visualize(video, output_file):
    ...
    """
    Sometimes labels with ids for certain events are overlapping with each other - this map allows to move them a little
    """
    EVENT_ID_POSTIONS = {
        # 573: (0, 0),
        # 574: (0, 0),
    ...
    for beh_id in beh_ids:
        for event in events:
            ...
            start_second = math.floor(event.start) - 4
            stop_second = math.ceil(event.stop) + 10
...
if __name__ == "__main__":
    ...
    visualize(video, output_file=f"{C.StatePoint.CHARTS_OUTPUT_DIR}/{video}-ids.png")
